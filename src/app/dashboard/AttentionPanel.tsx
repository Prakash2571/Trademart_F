...
export function AttentionPanel() {
  ...
  // GET /api/shopify/products sends ProductDto[] DIRECTLY as `data`, not wrapped
  // in { products }. Reading `.products` off an array yields undefined, so the
  // count silently stayed 0 and nothing ever appeared here - the exact contract
  // bug the review page documents.
  const review = useApi<ProductDto[]>(
  ...
  // ---- Review queue ---------------------------------------------------------
  const awaitingReview = review.data?.length ?? 0;
  if (awaitingReview > 0) {
    items.push({
      key: 'review',
